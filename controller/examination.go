...
// GET /examination/:id
func GetBExamination(c *gin.Context) {
	var examination []entity.Examination
	id := c.Param("id")
	if err := entity.DB().Preload("Patient").Preload("Doctor").Preload("Clinic").Preload("Disease").Preload("Medicine").Raw("SELECT * FROM examinations WHERE patient_id = ?", id).Find(&examination).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": examination})
