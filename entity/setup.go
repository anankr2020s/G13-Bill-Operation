...
func SetupDatabase() {
	...
	db.Raw("SELECT * FROM patients WHERE Id_card = ?", "1111111111111").Scan(&patient1)

	var patient2 Patient
	db.Raw("SELECT * FROM patients WHERE Id_card = ?", "2222222222222").Scan(&patient2)

	var patient3 Patient
	db.Raw("SELECT * FROM patients WHERE Id_card = ?", "3333333333333").Scan(&patient3)
	...
	db.Model(&Examination{}).Create(&Examination{
		...
		Treatment:     "ฉีดวัคซีน",
		TreatmentCost: 1500,
		Medicine:      Paracetamol500mg,
		MedicineCost:  150,
		TreatmentTime: time.Date(2020, 10, 10, 0, 0, 0, 0, time.UTC),
	})
	db.Model(&Examination{}).Create(&Examination{
		Patient:       patient2,
		Doctor:        Wichai,
	...
	//PatientRight Data
	Pr1 := PatientRight{
		Name:     "ไม่สิทธิ์การรักษา",
		Discount: 0,
	}
	db.Model(&PatientRight{}).Create(&Pr1)

	Pr2 := PatientRight{
		Name:     "สิทธิ์ประกันสุขภาพ",
		Discount: 80,
	}
